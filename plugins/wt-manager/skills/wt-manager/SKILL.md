...
`wt.zsh` はトップレベルに公開関数 `wt` **1つだけ**を定義し、各サブコマンド (`wt new`/`ls`/`set`/`rm`/`claude`/`cd`) の実体 `_wt_*` は `wt()` の中に内包する (理由は「内包設計」節)。

実体は **`~/dotfiles/dotfiles/wt.zsh` の1ファイルのみ** (プラグインに複製は同梱しない)。`.zshrc` から相対パスで source され、ユーザの対話シェルで `wt` が使えるのはこれが load されているため。**この skill は dotfiles リポジトリが初期セットアップ済みであること (= `wt.zsh` が source 済み) を前提とする。** dotfiles 未セットアップの環境では `wt` 自体が存在せず skill は機能しない。
...
実装の確認・編集はすべて `~/dotfiles/dotfiles/wt.zsh` (テストは同ディレクトリの `wt.test.zsh`) を見る。複製は無いので参照先はこの1ファイルのみ。
