...
fzf / `claude --bg` / `$EDITOR` 連動を除いた純粋ロジック + git 実操作を 59 アサーションで検証します (一時 git リポジトリを `mktemp` で作成し `trap` で削除)。internal ヘルパ (`_wt_*`) は `wt()` に内包されているため、テストは `WT_KEEP_INTERNAL=1` でヘルパを展開してから直接検証します。
