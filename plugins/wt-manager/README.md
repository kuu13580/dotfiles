...
| `wt new -b <branch> <dir> [base] [-d desc]` | 新規作成 + description 記録 (`<dir>` はディレクトリ名のみ、配置先は自動検出) |
...
fzf / `claude --bg` / `$EDITOR` 連動を除いた純粋ロジック + git 実操作を 68 アサーションで検証します (一時 git リポジトリを `mktemp` で作成し `trap` で削除)。internal ヘルパ (`_wt_*`) は `wt()` に内包されているため、テストは `WT_KEEP_INTERNAL=1` でヘルパを展開してから直接検証します。
