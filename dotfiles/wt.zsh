...
#
# Location  : ~/dotfiles/dotfiles/wt.zsh (唯一の実体。プラグインに複製は持たない)
...
function _wt_new() {
  ...
  if (( $# == 0 )) && [[ -t 0 ]]; then
    ...
    if (( ${#dir_cands} > 1 )); then
      ...
      dir=""
      vared -p "Directory name [$def_dir]: " dir
      dir="$(_wt_trim "$dir")"
      [[ -z "$dir" ]] && dir="$def_dir"
    fi
    base="$(_wt_pick_base)"
    desc=""
    vared -p 'Description (何用か): ' desc
    echo
    echo "  branch : $branch"
    echo "  dir    : $dir"
